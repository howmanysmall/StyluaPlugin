...
local function FastRequire(ModuleScript: ModuleScript)
	...
	if ModuleData ~= nil then
		...
		assert(ModuleData ~= nil, ModuleScript:GetFullName() .. " returns nil.")
		RequireCache[ModuleScript] = ModuleData
		return ModuleData
	end
end

local function Table_FastRemove(Array, Index)
	local Length = #Array
	...
	__index = function(self, Index)
		...
		for ParsedType in string.gmatch(Index .. "Or", "(%w-)Or") do -- Not the prettiest, but hey, we got parsing baby!
			...
			elseif string.find(ParsedType, "^EnumerationOfType") then
				...
				Types["Enumeration of type " .. ParsedType] = EnumerationType.Cast
...
local CustomTypes_PositiveInteger = CustomTypes.PositiveInteger
...
function Typer.AssignSignature(...)
	...
	if CustomTypes_PositiveInteger(..., type((...))) then
...
function Typer.PromiseAssignSignature(...)
	...
	for Index, ParameterSignature in ipairs(StackSignature) do
		if type(ParameterSignature) == "table" then
			...
			return Promise_Reject("Definition for parameter #" .. Index .. " must be a table")
		end
	end
	...
	if Castable then
		return function(...)
			...
			if not TypeSuccess then
				return Promise_Reject(table.unpack(Returned, 1, Returned.n))
			else
...
Typer.MapStrictDefinition = Typer.AssignSignature(Typer.Table, function(this)
	local self = this
...
Typer.MapDefinition = Typer.AssignSignature(Typer.Table, function(this)
	local self = this
	for _, Type in next, self do
		ExternalTransformTable(Type)
	end
