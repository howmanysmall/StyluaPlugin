...
local function makeErrorHandler(traceback)
	...
	return function(err)
		...
		return Error.new({
			...
			trace = debug_traceback(tostring(err), 2);
		})
	end
end
...
function Promise.new(executor)
	return Promise._new(debug_traceback(nil, 2), executor or NOOP)
end
...
function Promise.defer(callback)
	local traceback = debug_traceback(nil, 2)
	local promise
...
function Promise.resolve(...)
	...
	return Promise._new(debug_traceback(nil, 2), function(resolve)
...
function Promise.reject(...)
	local values = table.pack(...)
	return Promise._new(debug_traceback(nil, 2), function(_, reject)
...
function Promise.try(...)
	return Promise._try(debug_traceback(nil, 2), ...)
end
...
function Promise.all(promises)
	return Promise._all(debug_traceback(nil, 2), promises)
end
...
function Promise.some(promises, amount)
	...
	return Promise._all(debug_traceback(nil, 2), promises, amount)
end

function Promise.any(promises)
	return Promise._all(debug_traceback(nil, 2), promises, 1):andThen(function(values)
		return values[1]
	end)
end
...
function Promise.allSettled(promises)
	...
	return Promise._new(debug_traceback(nil, 2), function(resolve, _, onCancel)
		-- An array to contain our resolved values from the given promises.
...
function Promise.race(promises)
	...
	return Promise._new(debug_traceback(nil, 2), function(resolve, reject, onCancel)
		local newPromises = table.create(#promises)
...
function Promise.promisify(callback)
	return function(...)
		return Promise._try(debug_traceback(nil, 2), callback, ...)
	end
end
...
function Promise.prototype:spread(successHandler)
	...
	return self:_andThen(debug_traceback(nil, 2), successHandler, nil, true)
end
...
function Promise.prototype:andThen(successHandler, failureHandler)
	...
	return self:_andThen(debug_traceback(nil, 2), successHandler, failureHandler, false)
end
...
function Promise.prototype:catch(failureCallback)
	...
	return self:_andThen(debug_traceback(nil, 2), nil, failureCallback, false)
end

-- function Promise.prototype:tap(tapCallback)
-- 	assert(type(tapCallback) == "function", string.format(ERROR_NON_FUNCTION, "Promise:tap"))
-- 	return self:_andThen(debug_traceback(nil, 2), function(...)
...
function Promise.prototype:tap(tapCallback, tapCatch)
	...
	if tapCatch then
		...
		return self:_andThen(debug_traceback(nil, 2), tapCallback, tapCatch, false)
	else
		return self:_andThen(debug_traceback(nil, 2), tapCallback, nil, false)
	end
end
...
function Promise.prototype:finally(finallyHandler)
	...
	return self:_finally(debug_traceback(nil, 2), finallyHandler)
end
...
function Promise.prototype:finallyCall(callback, ...)
	...
	local values = table.pack(...)
	return self:_finally(debug_traceback(nil, 2), function()
		return callback(table.unpack(values, 1, values.n))
	end)
end
...
function Promise.prototype:done(finallyHandler)
	...
	return self:_finally(debug_traceback(nil, 2), finallyHandler, true)
end
