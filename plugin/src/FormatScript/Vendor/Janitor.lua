...
function Janitor.__index:AddPromise(PromiseObject)
	if not Promise_Is(PromiseObject) then
		error(string.format(NOT_A_PROMISE, typeof(PromiseObject), tostring(PromiseObject)))
	end

	if PromiseObject:GetStatus() == Promise.Status.Started then
		local Id = newproxy(false)
		local NewPromise = self:Add(Promise_Resolve(PromiseObject), "Cancel", Id)
...
**--]]
function Janitor.__index:LinkToInstance(LinkObject: Instance, AllowMultiple: boolean?)
	local Object = LinkObject
