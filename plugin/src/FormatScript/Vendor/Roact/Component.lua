...
function Component:setState(mapState)
	...
	if lifecyclePhase == ComponentLifecyclePhase.ShouldUpdate or lifecyclePhase == ComponentLifecyclePhase.WillUpdate or lifecyclePhase == ComponentLifecyclePhase.Render or lifecyclePhase == ComponentLifecyclePhase.WillUnmount then
...
function Component:__getContext(key)
	...
	return self[InternalData].virtualNode.context[key]
end
...
function Component:__validateProps(props)
	...
	if type(validator) ~= "function" then
		error(string.format("validateProps must be a function, but it is a %s.\nCheck the definition of the component %q.", typeof(validator), self.__componentName))
	end
	...
	if not success then
		...
		error(string.format("Property validation failed in %s: %s\n\n%s", self.__componentName, tostring(failureReason), self:getElementTraceback() or "<enable element tracebacks>"), 0)
	end
end
