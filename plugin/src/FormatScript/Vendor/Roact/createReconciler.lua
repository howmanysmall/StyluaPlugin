...
local function createReconciler(renderer)
	...
	local function updateVirtualNodeWithRenderResult(virtualNode, hostParent, renderResult)
		if Type.of(renderResult) == Type.Element or renderResult == nil or type(renderResult) == "boolean" then
			...
			error(string.format("%s\n%s", "Component returned invalid children:", virtualNode.currentElement.source or "<enable element tracebacks>"), 0)
		end
	end
	...
	function updateVirtualNode(virtualNode, newElement, newState)
		...
		if config.typeChecks then
			assert(Type.of(newElement) == Type.Element or type(newElement) == "boolean" or newElement == nil, "Expected arg #2 to be of type Element, boolean, or nil")
		end
	...
	local function createVirtualNode(element, hostParent, hostKey, context, legacyContext)
		if config.internalTypeChecks then
			...
			internalAssert(type(legacyContext) == "table" or legacyContext == nil, "Expected arg #5 to be of type table or nil")
		end

		if config.typeChecks then
			assert(hostKey ~= nil, "Expected arg #3 to be non-nil")
			assert(Type.of(element) == Type.Element or type(element) == "boolean", "Expected arg #1 to be of type Element or boolean")
		end
